...
def testBayes(sentencesTest, dataName, pWordPos, pWordNeg, pWord,pPos):
    ...
    Modelperformance(total, correct, totalpos, totalpospred, totalneg, totalnegpred, correctpos, correctneg )
...
def Modelperformance(total, correct, totalpos, totalpospred, totalneg, totalnegpred, correctpos, correctneg ):
        a=(correctpos/totalpospred)
        b=(correctpos/totalpos)
        
        Accuracy=(correct/total)
        Precision_pos=a
        Recall_pos=b     
        F_measure_pos=(2*a*b)/(a+b) 
        print("Accuracy:",Accuracy)
        print("Precision_pos:" ,Precision_pos)
        print("Recall_pos:" ,Recall_pos)
        print("Fmeasure_pos:" ,F_measure_pos)
        
        c=(correctneg/totalnegpred)
        d=(correctneg/totalneg)
        Precision_neg=c
        Recall_neg=d
        F_measure_neg=(2*c*d)/(c+d)
        print("Precision_neg:" ,Precision_neg)
        print("Recall_neg:", Recall_neg)
        print("Fmeasure_neg:", F_measure_neg)
...
def testDictionary(sentencesTest, dataName, sentimentDictionary, threshold):
    ...
    negation = {"not","never","none","nothing"}
    intensifier = {"definitely":2, "very":2, "extremely":3, "too":2, "so":1, "completely":3, "absolutely":3, "amazingly:":2, "quite":1 }
    diminisher = {"somewhat":1, "barely":2, "rarely":3, "fairly":1, "almost":2, "partly":1, "mostly":1, "kinda":1, "marginally":2, "moderately":2 }
    for sentence, sentiment in sentencesTest.items():
        ...
        for index,word in enumerate(Words):
            if word in sentimentDictionary:
               ...
               #diminsher rule
               if prev_word in diminisher:
                   if sentiment=="positive":
                       score=score-diminisher[prev_word]
                   else:
                       score=score+diminisher[prev_word]
               #intesifier rule
               if prev_word in intensifier:
                   if sentiment=="positive":
                       score=score+intensifier[prev_word]
                   else:
                       score=score-intensifier[prev_word]
               #Negation rule
               if prev_word in negation:
                    if sentiment=="positive":
                        score=(-1)*(score-1)
                    else:
                        score=(-1)*(score+1)
...
def mostUseful(pWordPos, pWordNeg, pWord, n):
    ...
    print ("\nPOSITIVE:")
    print (tail)
    compare(head, tail, n, sentimentDictionary)
    
def compare(head, tail, n, sentimentDictionary):
    z=0
    for z in range(0,n):
        if head[z] in sentimentDictionary:
            print(head[z])
    for z in range(0,n):
        if tail[z] in sentimentDictionary:
            print(tail[z])
